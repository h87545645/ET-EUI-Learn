...
public class FragAnimEvent : MonoBehaviour
{
    ...
    {
        EventSystem.Instance.Publish(Root.Instance.Scene, new FrogLanded(){});
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (!_hasTurn)
        {
            ...
            if (!isDown)
            {
                ...
                EventSystem.Instance.Publish(Root.Instance.Scene, new FrogCollisionEnter2D(){isUp = isUp , isLeft = isLeft});
                // if (isUp)
                // {
